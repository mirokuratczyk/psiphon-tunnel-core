...
func newFrontedHTTPClientInstance(
	...
	spec, dialParams, err :=
		ShuffleAndGetNetworkReplayParameters[parameters.FrontingSpec, frontedHTTPDialParameters](
			networkID,
			replayEnabled,
			frontingSpecs,
	...
	if err != nil {
		...
		// When there's an error, try to continue, using a random fronting spec
	...
	if !isReplay {
		dialParams, err = makeFrontedHTTPDialParameters(
			...
			payloadSecure)
		if err != nil {
			return nil, errors.TraceMsg(err, "makeFrontedHTTPDialParameters failed")
		}
