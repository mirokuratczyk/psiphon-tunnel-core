...
@objc class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    internal func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        // Choose which proxy to use. If unset, then WKWebView requests
        // are not proxied.
        self.useProxy = .http
...
extension AppDelegate: TunneledAppDelegate {
    ...
    func onListeningSocksProxyPort(_ port: Int) {
        ...
        if self.useProxy == .socks {

            NSLog("Configuring WKWebView to use SOCKS proxy")

            DispatchQueue.main.async {
                let endpoint = NWEndpoint.hostPort(
                    host: "127.0.0.1",
                    port: NWEndpoint.Port(rawValue:UInt16(port))!)
                let proxyConfig = ProxyConfiguration.init(socksv5Proxy: endpoint)
    ...
    func onListeningHttpProxyPort(_ port: Int) {
        ...
        if self.useProxy == .http {

            NSLog("Configuring WKWebView to use HTTP proxy")
