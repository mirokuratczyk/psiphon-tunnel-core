    ...
    NSString *activeInterface =
        [NetworkInterface getActiveInterfaceWithReachability:self->reachability
                                     andCurrentNetworkStatus:atomic_load(&self->currentNetworkStatus)
                                                       error:&err];
    if (err != nil) {
        [self logMessage:[NSString stringWithFormat:@"getNetworkID: error getting active interface %@", err.localizedDescription]];
        return kNetworkIDUnknown;
    }
    NSString *networkID = [NetworkID getNetworkID:[self->reachability reachabilityStatus]
                       defaultActiveInterfaceName:activeInterface
                                            error:&err];
    if (err != nil) {
        [self logMessage:[NSString stringWithFormat:@"getNetworkID: error getting network ID %@", err.localizedDescription]];
        return kNetworkIDUnknown;
    }
