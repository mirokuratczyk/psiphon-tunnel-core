...
- (void)pathUpdateHandler:(nw_path_t _Nonnull)path emitNotification:(BOOL)emitNotification API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0)) {
    ...
    if (self.pathState.path != nil && [DefaultRouteMonitor pathUpdateIsRedundant:self.pathState.path oldPath:path]) {
...
+(BOOL)pathUpdateIsRedundant:(nw_path_t)newPath oldPath:(nw_path_t) oldPath API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0)) {
    if (nw_path_is_equal(newPath, oldPath)) {
        return TRUE;
    }
    // nw_path_is_equal may return FALSE even though the paths are identical when comparing all the
    // information that can be gathered with the nw_path APIs. If this is the case, then the paths
    // can be considered equal and the new path update is redundant.
    return pathIsEqual(newPath, oldPath);
}
...
bool pathIsEqual(nw_path_t path, nw_path_t otherPath) API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0)) {
    ...
    nw_endpoint_t otherLocalEndpoint = nw_path_copy_effective_local_endpoint(otherPath);
    if (localEndpoint != nil && otherLocalEndpoint != nil) {
        if (!endpointIsEqual(localEndpoint, otherLocalEndpoint)) {
            return FALSE;
        }
    } else if (localEndpoint != otherLocalEndpoint) {
        return FALSE;
    }

    nw_endpoint_t remoteEndpoint = nw_path_copy_effective_remote_endpoint(path);
    nw_endpoint_t otherRemoteEndpoint = nw_path_copy_effective_remote_endpoint(otherPath);
    if (remoteEndpoint != nil && otherRemoteEndpoint != nil) {
        if (!endpointIsEqual(remoteEndpoint, otherRemoteEndpoint)) {
            return FALSE;
        }
    } else if (remoteEndpoint != otherRemoteEndpoint) {
        return FALSE;
    }
...
bool endpointIsEqual(nw_endpoint_t _Nonnull endpoint, nw_endpoint_t _Nonnull otherEndpoint) API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0)) {
    ...
    if (endpoint_type == nw_endpoint_type_url) {
        ...
        // Nothing to compare. Assume endpoints differ.
        return FALSE;
    }
...
bool sockaddr_is_equal(const struct sockaddr *addr1, const struct sockaddr *addr2) {
    if (addr1 != NULL && addr2 != NULL) {
        ...
        if (addr1->sa_family == AF_INET) {
            ...
            // Unsupported. Assume endpoints differ.
            return FALSE;
        }
    } else {
        return addr1 == addr2;
